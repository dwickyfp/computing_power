...
class LineageParser:
    ...
    def parse(self, sql: str | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"SQL parsing failed: {e}", sql=sql[:200] if sql else None)
    ...
    def _extract_ctes(self, parsed: "exp.Expression") -> list[dict[str, Any]]:
        ...
        for cte in with_clause.find_all(exp.CTE):
            ...
            # Extract source tables from the CTE query
            source_tables = []
            ...
            # Extract columns selected in the CTE
            cte_columns = []
            cte_select = cte_query.find(exp.Select)
            if cte_select:
                for expr in cte_select.expressions:
                    if isinstance(expr, exp.Alias):
                        cte_columns.append(expr.alias)
                    elif isinstance(expr, exp.Column):
                        cte_columns.append(expr.name)
    ...
    def _extract_output_columns(
        ...
        for expr in select.expressions:
            if isinstance(expr, exp.Star):
                # Handle SELECT * - use source columns if available
                for col in self.source_columns:
...
def parse_lineage(
    ...
    Args:
        ...
        source_columns: List of source column names (for SELECT * expansion)
